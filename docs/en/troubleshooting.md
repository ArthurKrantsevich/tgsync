...
Messages from tgsync are in Russian; the quoted strings below are what you will see, with a translation.
...
`tgsync check` prints `✗ конфиг — …` ("config"), and `tgsync run` exits. All problems are listed at once.

| Message | Cause | Fix |
|---|---|---|
| `TELEGRAM_BOT_TOKEN is required` (also `ALLOWED_USER_IDS`, `GROUP_CHAT_ID`, `PROJECTS_ROOT`) | Variable missing or empty, **or `.env` was not found at all** | Check the folder on the first line of `tgsync check` (`папка …`). Put `.env` there, or set `TGSYNC_HOME`. See [node folder](configuration.md#node-folder). |
...
| Symptom | Cause | Fix |
|---|---|---|
| `✗ бот — токен не принят или Telegram недоступен` ("token rejected or Telegram unreachable"), log shows `Unauthorized` | Wrong or revoked token, extra spaces or quotes | Copy the token again from @BotFather (`/token`). |
...
| `✗ группа с темами — группа … недоступна: бот не добавлен или неверный GROUP_CHAT_ID` ("group unavailable: bot not added or wrong id"), log `chat not found` | Bot is not a member, or the id is wrong/outdated | Add the bot to the group. Re-read the id **after** enabling Topics: converting to a supergroup changes it. |
| `✗ группа с темами — в группе не включены темы (Topics)` | Topics are off | Group settings → Topics → on. Then re-check the id. |
| `✗ права бота — сделай бота администратором с правом «Управление темами»` ("make the bot an admin with Manage topics"); `run` exits with `бот должен быть администратором группы с правом «Управление темами» (can_manage_topics)` | Bot is a plain member, or the right is off | Administrators → your bot → enable **Manage topics**. |
| `✓ права бота — управление темами есть; стоит добавить …` ("worth adding …") and a `🔧` notice in the control topic | Optional rights missing | Add Pin messages / Change group info / Delete messages ([setup 2.3](setup.md#23-make-the-bot-an-administrator)). |
| No pinned card | No **Pin messages** right | Add it and restart. |
| Empty topics are closed, not deleted; 🧹 Cleanup only closes | No **Delete messages** right | Add it. |
...
| Windows: `найден claude.cmd (обёртка npm), он не запускается` ("found claude.cmd (npm shim), it does not start") | Claude Code installed via npm | Install with the native installer (`irm https://claude.ai/install.ps1 \| iex`) or set `CLAUDE_CLI_PATH` to a `claude.exe`. |
| Windows: `claude.exe не найден в PATH` | Not installed or not on `PATH` of your logon session | Install, log off and on, run the installer again. |
...
| `❌ Процесс claude завершился во время хода` ("claude process exited during the turn") | `claude` crashed or was killed (OOM, update in progress) | Send another message to continue. Check memory; lower `MAX_PARALLEL_SESSIONS`. |
| `⏸ Нода перезапустилась во время хода` ("node restarted during the turn") | Service restart or upgrade | Send a message to continue. |
...
| `эта сессия уже открыта в другой теме` ("this session is already open in another topic") | You tried to resume a session that already has a topic | Use the existing topic. |
...
| Symptom | Cause | Fix |
|---|---|---|
| `make install` stops with `Запущен другой процесс tgsync` ("another tgsync process is running") | A terminal `./bin/tgsync run` or a stray process | Stop it (`pkill -x tgsync` after checking `pgrep -a tgsync`), run again. |
| `make install` stops with `Заполни … .env и запусти make install ещё раз` ("fill in .env and run again") | No `.env` found; the template was copied to the node folder | Edit that file and run `make install` again. |
| `make install` stops with `Исправь ошибки выше` ("fix the errors above") | `tgsync check` failed | Fix the `✗` lines. |
| `Нет Go и нет готового tgsync рядом со скриптом` ("no Go and no binary next to the script") | Neither Go nor a release binary | Install Go, or use a release archive. |
...
| `sudo: сокет …: …` ("sudo: socket …") | Could not create `askpass.sock` (stale file, permissions) | Stop the node, delete the stale `askpass.sock` in the node folder, start again. |
...
| `✓ sudo — SUDO_MODE=off: команды с sudo отклоняются` and sudo commands are rejected | Default | Enable a mode ([configuration → sudo](configuration.md#sudo)) or use `NOPASSWD`. |
| `✗ sudo — SUDO_MODE=telegram: дай боту право «Удаление сообщений»` ("give the bot Delete messages") | Required so the password is deleted | Add the right. |
| `✓ sudo — недоступен на Windows, SUDO_MODE принудительно off` | Windows has no sudo | Expected. |
...
|---|---|---|
| `🎙 Распознавание не настроено (STT_URL).` ("recognition not configured") | `STT_URL` empty | Set it and restart ([configuration → voice](configuration.md#voice-messages)). |
| `🎙 Голосовые работают в теме сессии.` ("voice works in a session topic") | Sent in the control topic | Send it in a session topic. |
| `🎙 Слишком длинное: N с, максимум M с.` ("too long") | Longer than `STT_MAX_SECONDS` | Raise the limit or split the message. |
| `⚠️ Аудио больше 20 МБ …` | Telegram does not let bots download larger files | Send a shorter recording. |
...
| `⚠️ STT: непонятный ответ сервера` ("unexpected server response") | `STT_URL` points to something that is not an OpenAI-compatible STT API | Point it to the server root, without `/v1`. |

## Files

| Message | Cause | Fix |
|---|---|---|
| `…: файл вне папки проекта` / `ссылка ведёт за пределы проекта` ("outside the project" / "link leads outside") | `/file` only serves files inside the project | Copy the file into the project. |
| `…: служебный файл tgsync не отправляется` ("tgsync service file is not sent") | `.env` and the database are never sent | Expected. |
| `… МБ, больше лимита Telegram 50 МБ` | Bot upload limit | Compress or split. |
| `файл больше 20 МБ — лимит Telegram для ботов` | Bot download limit for files you send | Send a smaller file, or put it on the machine another way. |
